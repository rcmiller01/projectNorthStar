#!/usr/bin/env python3
"""Create working views using TO_JSON_STRING approach."""
...
print("🏗️ Creating working dashboard views...")

# Use TO_JSON_STRING to convert JSON to string, then parse it
view_common_issues = """
CREATE OR REPLACE VIEW `gleaming-bus-468914-a6.demo_ai.view_common_issues` AS
SELECT 
    JSON_EXTRACT_SCALAR(TO_JSON_STRING(meta), '$.category') as issue_type,
    JSON_EXTRACT_SCALAR(TO_JSON_STRING(meta), '$.severity') as severity,
...
WITH content_counts AS (
    SELECT 
        ...
        ARRAY_AGG(JSON_EXTRACT_SCALAR(TO_JSON_STRING(meta), '$.severity')) as severities
    FROM `gleaming-bus-468914-a6.demo_ai.chunks`
    WHERE meta IS NOT NULL
    GROUP BY content
    HAVING COUNT(*) > 1
...
print("\n🎯 Views created successfully! Dashboard should now work.")
